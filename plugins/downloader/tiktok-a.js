...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        ...
        await conn.sendFile(m.chat, au.data, meta.title + '.mp3', '', m, /vn/.test(args[1]), { mentions: [m.sender] })
    } catch {
        try {
            ...
            let a = await conn.getBuffer(v)
            let au = await toAudio(a, 'mp4')
            await conn.sendFile(m.chat, au.data, description + '.mp3', '', m, /vn/.test(args[1]), { mentions: [m.sender] })
        }  catch (e) {
            throw e
        }
    }
